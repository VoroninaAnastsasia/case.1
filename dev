...
def getTextStats(text: str) -> dict:
    """
    ...
    # Функции подсчёта слогов для разных языков.
    ...
    # Если язык неизвестен, используем английский алгоритм как резервный вариант.

    if syllable_function is None:

        syllable_function = countSyllablesEn
...
def rareWordDensity(text, freq_dict):
    """
...
def analyzeText(text, freq_dict):
    """
    ...
    # Если словарь не передан, используем пустой словарь.
    ...
    interpretation = interpretFlesch(
        flesch
    )
...
def printResults(result):
    ...
    print(
        ...
        f"{result['lexicalDiversity']:.2f}"
    )

    rare_density = (
        result["rareWordDensity"]
    )
